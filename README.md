...
| 🔍 **Два источника вакансий** | hh.ru (фильтры по городу, опыту, графику; нужен токен) и Хабр Карьера (`--source habr`, работает без токена). |
...
jobsmith search <query> --source habr   # Поиск на Хабр Карьере (без токена)
