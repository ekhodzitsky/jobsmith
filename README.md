...
| 🔍 **Четыре источника вакансий** | hh.ru (нужен токен) и три без токена: Хабр Карьера (`--source habr`), Работа России (`--source trudvsem`, открытое гос. API с зарплатами) и GeekJob (`--source geekjob`). |
...
jobsmith search <query>     # Поиск вакансий (hh.ru по умолчанию)
jobsmith search <query> --source habr|trudvsem|geekjob   # Без токена
jobsmith apply <url|id>     # AI-пайплайн: оценка → CV → письмо → PDF (источник по URL)
...
- **HH API без токена.** `api.hh.ru` отвечает `403 forbidden` на анонимные
  запросы — `search`/`apply` для hh.ru требуют OAuth-авторизации приложения,
  которой в jobsmith пока нет. **Обходной путь:** используйте Хабр Карьеру
  (`jobsmith search <query> --source habr`) — она работает без токена через
  публичные RSS и schema.org JSON-LD.
- **`salary --role/--area`** — партнёрский эндпоинт HH (см. выше).
- **Rabota.ru не поддержан.** Сайт — Nuxt SPA: листинг вакансий доступен
  только из обфусцированного `window.__NUXT__` (привязка к конкретной
  сборке, нестабильно), публичного API нет. Стабильного машинного канала
  для анонимного поиска не нашлось — источник намеренно не добавлен.
